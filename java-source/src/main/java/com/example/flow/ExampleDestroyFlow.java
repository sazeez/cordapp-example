 ...
 */
public class ExampleDestroyFlow {
    @InitiatingFlow
    @StartableByRPC
    public static class Initiator extends FlowLogic<SignedTransaction> {

        private final IOUState iouState;
        ...
        public Initiator(IOUState iouState, Party otherParty) {
...
//                    .addOutputState(iouState, IOU_CONTRACT_ID) TODO: How to send no state?
        ...
        public SignedTransaction call() throws FlowException {
            class SignTxFlow extends SignTransactionFlow {
                ...
                protected void checkTransaction(SignedTransaction stx) {
                    requireThat(require -> {
                        try {
                            // Get inputs from service hub TODO: Is this the correct approach to get inputs?
                            ContractState input = getServiceHub().loadState(stx.getTx().getInputs().get(0)).getData();
                            require.using("This must be an IOU transaction.", input instanceof IOUState);
                            IOUState iou = (IOUState) input;
                            require.using("I won't accept IOUs with a value over 100.", iou.getValue() <= 100);

                            //TODO: check for outputs?
